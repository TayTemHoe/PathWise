...
import 'package:path_wise/model/user_profile.dart'; // Contains UserModel
...
class PreviewResumePage extends StatelessWidget {
  ...
  Widget _buildResumePreview(BuildContext context) {
    ...
    final englishTests = profileVM.englishTests; // ✅ Added English Tests
    ...
    return Container(
      ...
      child: ClipRRect(
        borderRadius: BorderRadius.circular(16),
        child: _buildTemplateLayout(profile, skills, education, experience, englishTests),
      ),
    );
  }

  Widget _buildTemplateLayout(
      UserModel profile, // ✅ Changed UserProfile to UserModel
      List<Skill> skills,
      List<AcademicRecord> education,
      List<Experience> experience,
      List<EnglishTest> englishTests, // ✅ Added parameter
      ) {
    switch (resume.template) {
      case ResumeTemplateType.tech:
        return _buildTechTemplate(profile, skills, education, experience, englishTests);
      case ResumeTemplateType.business:
        return _buildBusinessTemplate(profile, skills, education, experience, englishTests);
      case ResumeTemplateType.creative:
        return _buildCreativeTemplate(profile, skills, education, experience, englishTests);
      case ResumeTemplateType.academic:
        return _buildAcademicTemplate(profile, skills, education, experience, englishTests);
    }
  }
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                // ✅ Added: English Proficiency
                if (englishTests.isNotEmpty) ...[
                  _buildSectionHeader('ENGLISH PROFICIENCY', secondaryColor),
                  const SizedBox(height: 16),
                  ...englishTests.map((test) => _buildTechEnglishTest(test, secondaryColor)),
                  const SizedBox(height: 28),
                ],
  ...
  Widget _buildTechEnglishTest(EnglishTest test, Color color) {
    return Container(
  ...
  Widget _buildTechEducation(AcademicRecord edu, Color color) {
    final programTitle = edu.programName ?? edu.major ?? edu.level; // ✅ Updated
    return Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Wrap(
                  spacing: 10,
                  children: [
                    if (edu.level.isNotEmpty) Text(edu.level, style: TextStyle(fontSize: 12, color: Colors.grey[600])),
                    if (edu.cgpa != null) Text('CGPA: ${edu.cgpa}', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: color)),
                  ],
                )
              ],
            ),
        ...
        children: [
          ...
          Column(
            ...
            children: [
              ...
              // ✅ Skills (Prioritized as requested)
              if (resume.sections.skills && skills.isNotEmpty) ...[
              ...
              // ✅ Added: English Tests for Business
              if (englishTests.isNotEmpty) ...[
                _buildBusinessSection('LANGUAGES & TESTS', secondaryColor),
                const SizedBox(height: 16),
                ...englishTests.map((test) => _buildTechEnglishTest(test, secondaryColor)),
                const SizedBox(height: 28),
              ],
        ...
        Container(
          ...
          child: Column(
            children: [
              ...
              // ✅ Updated Contact: Phone + Location added
              if (resume.sections.personalInfo) ...[
                const Text(
                  'CONTACT',
                  style: TextStyle(
                ...
                const SizedBox(height: 8),
                if (profile.city != null || profile.country != null)
                  _buildCreativeContact(Icons.location_on, '${profile.city ?? ''}, ${profile.country ?? ''}'),
                const SizedBox(height: 32),
              ],
              ...
              // ✅ Added: English Tests in Sidebar
              if (englishTests.isNotEmpty) ...[
                const Text(
                  'LANGUAGES',
                  style: TextStyle(
        ...
        children: [
          ...
          if (resume.sections.personalInfo) ...[
            ...
            Wrap(
              ...
              children: [
                ...
                if (profile.city != null) Text('${profile.city}, ${profile.country}'),
              ],
            ),
          ],
          const SizedBox(height: 24),
          ...
          Column(
            ...
            children: [
              ...
              if (resume.sections.skills && skills.isNotEmpty) ...[
                _buildAcademicSection('SKILLS & EXPERTISE', secondaryColor),
                const SizedBox(height: 16),
                Wrap(
                  spacing: 12,
                  runSpacing: 8,
                  children: skills.map((skill) => Text('• ${skill.name}')).toList(),
                ),
                const SizedBox(height: 28),
              ],
              ...
              // ✅ Added: English Tests
              if (englishTests.isNotEmpty) ...[
                _buildAcademicSection('LANGUAGES', secondaryColor),
                const SizedBox(height: 16),
                ...englishTests.map((t) => _buildTechEnglishTest(t, const Color(0xFF374151))),
                const SizedBox(height: 28),
              ],
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Added CGPA display for academic
          if (edu.cgpa != null)
            Text(
              'CGPA: ${edu.cgpa}',
  ...
  // ... [Keep other existing methods like _buildAcademicSection, _buildAcademicExperience, _buildSectionHeader, etc. exactly as they were] ...
  ...
  Widget _buildActionButtons(BuildContext context) {
    final profileVM = context.watch<ProfileViewModel>();
    final englishTests = profileVM.englishTests;
              ...
              children: [
                ...
                Expanded(
                  child: ElevatedButton.icon(
                    onPressed: viewModel.isDownloading ? null : () => _handleDownload(context, viewModel, englishTests),
  ...
  Future<void> _handleDownload(BuildContext context, ResumeViewModel viewModel, List<EnglishTest> englishTests) async {
    ...
    final path = await viewModel.downloadResume(resume, englishTests);
